...
mwan3_create_iface_route()
{
	...
	[ -n "$id" ] || return 0
		...
		$route_args=`ip route | grep $1 |grep default | awk '{print $3}'`
...
mwan3_track()
{
	local track_ip track_ips pid data it item sdata row
...
mwan3_set_policy()
{
	local iface_count id iface family metric probability weight it item data
...
#	config_get weight $1 weight 1
	...
	data=`cat /tmp/mwan3.json | jq .`
        ...
        it=0
        while true; do
                item=`echo $data | jq -r .member[$it]`
                [ "$item" = "null" ] && break || it=$((it+1))
                _jqs() {
                        echo ${item} | jq -r $1
                }
                [ $1 = $(_jqs .name) ] && {
                        iface=$(_jqs .interface)
                        [ ${iface} = "null" ] && iface=""
                        metric=$(_jqs .metric)
                        [ ${metric} = "null" ] && metric=1
                        weight=$(_jqs .weight)
...
{
	local last_resort lowest_metric_v4 lowest_metric_v6 total_weight_v4 total_weight_v6 policy IPT sd row

	policy=`echo $1 | jq -r .name`

	#config_get last_resort $1 last_resort unreachable
	last_resort=`echo $1 | jq -r .last_resort`
	[ ${last_resort} = "null" ] && last_resort=unreachable
	...
	#config_list_foreach $1 use_member mwan3_set_policy
        sd=`echo $1 | jq -r .use_member`
        for row in $(echo "$sd" | jq -r .[]); do
                mwan3_set_policy "$row"
        done
}
...
mwan3_set_policies_iptables()
{
	...
	while true; do
		...
		mwan3_create_policies_iptables "$item"
	done
}
...
{
	local ipset family proto policy src_ip src_port sticky dest_ip dest_port use_policy timeout rule policy IPT it item name data

	#rule="$1"
	rule=`echo $1 | jq -r .name`

	#config_get sticky $1 sticky 0
        sticky=`echo $1 | jq -r .sticky`
        [ ${sticky} = "null" ] && sticky=0

#	config_get timeout $1 timeout 600
        timeout=`echo $1 | jq -r .timeout`
        [ ${timeout} = "null" ] && timeout=600

	#config_get ipset $1 ipset
        ipset=`echo $1 | jq -r .ipset`
        [ ${ipset} = "null" ] && ipset="ipset"

#	config_get proto $1 proto all
        proto=`echo $1 | jq -r .proto`
        [ ${proto} = "null" ] && proto="all"

#	config_get src_ip $1 src_ip 0.0.0.0/0
        src_ip=`echo $1 | jq -r .src_ip`
        [ ${src_ip} = "null" ] && src_ip="0.0.0.0/0"

#	config_get src_port $1 src_port 0:65535
        src_port=`echo $1 | jq -r .src_port`
        [ ${src_port} = "null" ] && src_port="0:65535"

#	config_get dest_ip $1 dest_ip 0.0.0.0/0
        dest_ip=`echo $1 | jq -r .dest_ip`
        [ ${dest_ip} = "null" ] && dest_ip="0.0.0.0/0"

#	config_get dest_port $1 dest_port 0:65535
        dest_port=`echo $1 | jq -r .dest_port`
        [ ${dest_port} = "null" ] && dest_port="0:65535"

#	config_get use_policy $1 use_policy
        use_policy=`echo $1 | jq -r .use_policy`
        [ ${use_policy} = "null" ] && use_policy="use_policy"

#	config_get family $1 family any
        family=`echo $1 | jq -r .family`
        [ ${family} = "null" ] && family="any"
		...
		if [ "$use_policy" == "default" ]; then
			...
			if [ "$sticky" -eq 1 ]; then
				...
				while true; do
					...
					mwan3_set_sticky_iptables "$name"
				done
		...
		if [ "$family" == "any" ]; then
			...
			for IPT in "$IPT4" "$IPT6"; do
				case $proto in
					...
					;;
					*)
					$IPT -A mwan3_rules -p $proto -s $src_ip -d $dest_ip $ipset -m mark --mark 0/$MMX_MASK -m comment --comment "$rule" -j $policy &> /dev/null
					;;
				esac
			done
			...
			case $proto in
				...
				;;
				*)
				$IPT4 -A mwan3_rules -p $proto -s $src_ip -d $dest_ip $ipset -m mark --mark 0/$MMX_MASK -m comment --comment "$rule" -j $policy &> /dev/null
				;;
			esac
...
mwan3_set_user_rules()
{
	...
	it=0
	while true; do
		item=`echo $data | jq -r .rule[$it]`
		[ "$item" = "null" ] && break || it=$((it+1))
		mwan3_set_user_iptables_rule "$item"
	done
